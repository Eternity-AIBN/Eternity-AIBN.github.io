...
1. 排序 + 插入，注意应该是**倒插！！！**
   ...
   为何？比如：$a≤b≤c≤d$，正常处理为：$a,c,b,d$，$b$和$c$在一块，可能出现问题；若倒插，则为$b,d,a,c$，中间两个数为$a$和$d$，差距比$b$和$c$大，不会出现问题。
...
直接循环n次会超时，考虑到一直是乘以同一个数（$x^n=x\times x\times x\times x...$），可以这样计算：$x^n=x^{\frac{1}{2}}\times x^{\frac{1}{2}}=...$，将复杂度从$O(n)$降为$O(log_2n)$
